class OrderMailer < ApplicationMailer
  ...
  def return_order(order, current_user)
    ...
    mail to: ENV['ACTION_MAILER_SEND_TO'], subject: "#{pluralize(@order.quantity, 'item')} borrowed by #{@order.member.name} have been marked as returned from #{@user.name}"
  end
  ...
  def cancel_order(order, current_user)
    ...
    mail to: ENV['ACTION_MAILER_SEND_TO'], subject: "An order regarding #{pluralize(@order.quantity, 'item')} borrowed by #{@order.member.name} has been canceled from #{@user.name}"
  end
end
